...
public class PlayerData {
        ...
        if (!playerDataFile.exists()) {
            try {
                if (!playerDataFile.createNewFile()) {
                    logger.log(Level.WARNING, "{Snake 2.0.0} [PlayerData.java] Failed to create PlayerData.yml file. It may already exist.");
                }
            } catch (IOException e) {
                logger.log(Level.SEVERE, "{Snake 2.0.0} [PlayerData.java] An error occurred while creating PlayerData.yml", e);
            }
        }
    ...
    public void saveConfig() {
        try {
            ...
            logger.log(Level.SEVERE, "{Snake 2.0.0} [PlayerData.java] An error occurred while saving PlayerData.yml", e);
        }
    }
}
