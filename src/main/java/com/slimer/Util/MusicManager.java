    ...
    public void startMusic(Player player) {
        ...
        if (!songFolder.exists()) {
            if (!songFolder.mkdirs()) {
                Bukkit.getLogger().severe("{Snake 2.0.0} [MusicManager.java] Failed to create songs folder. It may already exist.");
            }
        }

        // Check if song file exists
        if (!songFile.exists()) {
            Bukkit.getLogger().warning("{Snake 2.0.0} [MusicManager.java] No song file found at " + fullPath + ". Music will not be played.");
            return;
        }
        ...
        try {
            ...
            if (song == null) {
                Bukkit.getLogger().severe("{Snake 2.0.0} [MusicManager.java] Error loading the music file. Please ensure it's a valid NBS file.");
                return;
            }
            ...
            Bukkit.getLogger().severe("{Snake 2.0.0} [MusicManager.java] Could not load song from file " + fullPath + ": " + e.getMessage());
        }
    }
