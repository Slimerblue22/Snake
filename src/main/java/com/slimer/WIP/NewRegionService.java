 ...
 */
public class NewRegionService {
    private static NewRegionService instance;
    private Logger logger;
    private Connection connection;

    private NewRegionService(JavaPlugin plugin) {
        initializeDatabase(plugin);
        NewRegionHelpers.initializeInstance(connection, logger);
    }
    ...
    public static synchronized void initializeInstance(JavaPlugin plugin) {
        if (instance == null) {
            instance = new NewRegionService(plugin);
        }
    }

    /**
     * Retrieves the current instance of the NewRegionService.
     *
     * @return The current instance of the NewRegionService.
     * @throws IllegalStateException if the service has not been initialized.
     */
    public static synchronized NewRegionService getInstance() {
        if (instance == null) {
            throw new IllegalStateException("NewRegionService must be initialized with a JavaPlugin instance before use.");
        }
        return instance;
    }
    ...
    public boolean linkRegions(String regionName1, String regionName2) {
        try {
            ...
            int newLinkID = NewRegionHelpers.getInstance().generateUniqueLinkID();
